...
uint256 hashGenesisBlock("0xf4bd139347d274ff7ad65e6db3ee1676850b4c77f9af62bddc221e37712e60a9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Caymancoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Caymancoin Signed Message:\n";
...
{
    // Caymancoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 2333600 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 2336000 ); // Caymancoin: 584000 blocks/year

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 5 * 24 * 60 * 60; // Caymancoin: 5 days
static const int64 nTargetSpacing = 0.9 * 60; // Caymancoin: 90 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Caymancoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // Caymancoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block  //JIMMY
        const char* pszTimestamp = "Caymancoin the genesis of new the financial pipeline.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
       ...
       // txNew.vout[0].scriptPubKey = CScript() << ParseHex("040184710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        ...
        block.nVersion = 1;
        block.nTime    = 1388800819;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1390067097;
        ...
        assert(block.hashMerkleRoot == uint256("0x3de821b8034f1cbffdd89d48c0034eaeb00dc4d445d72cb1198fa368324a9bda"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Caymancoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CaymancoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CaymancoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CaymancoinMiner(CWallet *pwallet)
{
    printf("CaymancoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("caymancoin-miner");
    ...
    try { loop {
        ...
        printf("Running CaymancoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CaymancoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CaymancoinMiner, pwallet));
}
